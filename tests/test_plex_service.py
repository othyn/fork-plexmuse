...
def test_create_curated_playlist(plex_service, mock_plex_server):
    ...
    mock_playlist = MagicMock(title="New Playlist")
    mock_server.return_value.createPlaylist.return_value = mock_playlist
    ...
    mock_playlist.edit.assert_called_once_with(summary="Generated by Plexmuse")

    # Reset mocks
    mock_server.return_value.createPlaylist.reset_mock()
    mock_playlist.edit.reset_mock()

    # Test with prompt and model
    prompt = "Create a rock playlist"
    model = "gpt-4"
    playlist = plex_service.create_curated_playlist("New Playlist", track_recommendations, prompt=prompt, model=model)
    assert playlist.title == "New Playlist"
    mock_server.return_value.createPlaylist.assert_called_once()
    mock_playlist.edit.assert_called_once_with(summary=f"Generated by Plexmuse\nPrompt: {prompt}\nModel: {model}")
    ...
    playlist = plex_service.create_curated_playlist("New Playlist", track_recommendations, prompt=prompt)
    assert playlist.title == "New Playlist"
    mock_server.return_value.createPlaylist.assert_called_once()
    mock_playlist.edit.assert_called_once_with(summary=f"Generated by Plexmuse\nPrompt: {prompt}")
    ...
    playlist = plex_service.create_curated_playlist("New Playlist", track_recommendations, model=model)
    assert playlist.title == "New Playlist"
    mock_server.return_value.createPlaylist.assert_called_once()
    mock_playlist.edit.assert_called_once_with(summary=f"Generated by Plexmuse\nModel: {model}")
...
def test_create_curated_playlist_no_matches(plex_service, mock_plex_server):
    ...
    model = "gpt-4"
    with pytest.raises(ValueError, match="No tracks could be matched from recommendations"):
        plex_service.create_curated_playlist("New Playlist", track_recommendations, prompt=prompt, model=model)
